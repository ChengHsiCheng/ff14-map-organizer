...
function App() {
    ...
    let pool = lines.map((line) => {
      ...
      if (match) {
        ...
        mapDef.points.forEach(p => {
          ...
          if (d < bestTp.dist) bestTp = { name: p.name, dist: d };
        });

        return { player: playerName, mapName, x, y, closestPoint: bestTp.name, priority: mapDef.priority };
      }
      return null;
  ...
  const performCopy = (index) => {
    ...
    if (index > 0) {
      ...
      if (prevItem.closestPoint === item.closestPoint && prevItem.mapName === item.mapName) pointLabel = "繼續";
    }
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', background: '#0f0f0f', color: '#e0e0e0', fontFamily: '"Microsoft JhengHei", sans-serif', overflow: 'hidden' }}>
        ...
        <div style={{ background: '#252525', padding: '16px', borderRadius: '8px', border: '1px solid #333', height: '210px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {currentSettings.map((map, idx) => (
              <div key={idx} style={{ fontSize: '13px', color: '#ccc', padding: '6px 8px', borderRadius: '4px', background: idx % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'transparent', display: 'flex', alignItems: 'center' }}>
      ...
      <div style={{ flex: 1, padding: '30px', display: 'flex', flexDirection: 'row', gap: '24px', boxSizing: 'border-box', overflow: 'hidden', position: 'relative' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <textarea value={rawData} onChange={(e) => setRawData(e.target.value)} placeholder="貼上座標清單..."
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', alignItems: 'center', flexShrink: 0 }}>
            ...
            <button onClick={nextTargetAndCopy} disabled={parsedData.length === 0} style={{ fontSize: '18px', padding: '10px 20px', background: '#ffa726', color: '#000', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', opacity: parsedData.length === 0 ? 0.5 : 1 }}>下一位 ➡</button>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px' }}>
                <thead style={{ position: 'sticky', top: 0, background: '#252525', zIndex: 1 }}>
                  <tr><th style={{ padding: '12px 16px', color: '#ffa726', width: '50px' }}>#</th><th style={{ padding: '12px 16px' }}>玩家</th><th style={{ padding: '12px 16px' }}>區域</th><th style={{ padding: '12px 16px' }}>最近傳送點</th><th style={{ padding: '12px 16px' }}>座標</th></tr>
                </thead>
                <tbody>
