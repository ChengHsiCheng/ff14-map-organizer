  ...
  const parsedData = useMemo(() => {
    ...
    let pool = lines.map((line) => {
      ...
      if (match) {
        ...
        mapDef.points.forEach(p => {
          ...
          if (d < bestTp.dist) bestTp = { name: p.name, dist: d, x: p.x, y: p.y };
        });

        return {
          player: playerName, mapName, x, y, closestPoint: bestTp.name,
          tpDist: bestTp.dist, tpX: bestTp.x, tpY: bestTp.y, priority: mapDef.priority
        };
      }
      return null;
    ...
    const mapsInPool = [...new Set(pool.map(p => p.mapName))].sort((a, b) => {
      if (startPoint) {
        ...
        if (b === startPoint.mapName) return 1;
      }
      const pA = currentSettings.find(m => m.name === a)?.priority || 99;
      const pB = currentSettings.find(m => m.name === b)?.priority || 99;
      return pA - pB;
    });
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexShrink: 0 }}>
            ...
            <div style={{ width: '320px' }}>
              ...
              <input type="text" value={currentPosRaw} onChange={(e) => setCurrentPosRaw(e.target.value)} placeholder="貼上座標或聊天紀錄..."
          ...
          <textarea value={rawData} onChange={(e) => setRawData(e.target.value)} placeholder="貼上聊天內容清單..."
